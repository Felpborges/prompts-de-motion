# Skill Prompts de Motion

**Uma skill de Felipe Borges para o Claude Code.**

Ela escreve prompts de vídeo de motion design de 15 segundos (lançamento de produto, vídeo de marca, promo, explainer) prontos para colar no MiniMax H3 ou em modelos parecidos de texto para vídeo. Cada prompt sai com uma ideia central, paleta travada, roteiro com os tempos de cada momento, lista exata de textos e um Plano B para o que tiver mais chance de dar errado.

A skill escreve o prompt. A geração do vídeo acontece na ferramenta que você usar (Higgsfield, o site do MiniMax ou outra).

---

## Como instalar (jeito mais fácil)

1. Abra o **Claude Code**.
2. Clique no botão de copiar do bloco abaixo e cole tudo numa mensagem.
3. Envie. O Claude Code cria a skill para você.
4. Abra uma **sessão nova** do Claude Code.

~~~~text
Crie uma skill do Claude Code. Salve o conteúdo abaixo, exatamente como está, sem mudar nada, no arquivo ~/.claude/skills/prompts-de-motion/SKILL.md (crie as pastas se não existirem). Se já existir um arquivo nesse caminho, me avise antes de sobrescrever. No fim, me confirme o caminho do arquivo criado.
...
## Como instalar (jeito manual)

Se preferir criar o arquivo você mesmo:

1. Crie a pasta da skill:
   - **Mac/Linux:** `~/.claude/skills/prompts-de-motion/`
   - **Windows:** `C:\Users\SEU-USUARIO\.claude\skills\prompts-de-motion\`
2. Dentro dela, crie um arquivo chamado `SKILL.md`.
3. Copie o bloco abaixo e cole no arquivo. Salve.
4. Abra uma sessão nova do Claude Code.
...
Numa sessão nova do Claude Code, digite `/prompts-de-motion` seguido do pedido, ou só descreva o vídeo. Ela também entra sozinha quando você pede um vídeo de lançamento ou de motion. Exemplos:

- `/prompts-de-motion vídeo de lançamento de um fone de ouvido novo, estilo Apple`
- `crie um motion graphic de 15s sobre a evolução do Bitcoin nos últimos 10 anos`
- `me dá 5 ideias de motion para lançar meu curso`

Depois do primeiro prompt, dá para pedir ajustes:

- `refaz para um tênis de corrida`: mantém o estilo e troca o tema
- `muda o estilo para claymation`: troca o jeito de mover e de renderizar
- `muda as cores para azul e laranja`: reconstrói a paleta inteira
- `faz em 9:16`: recompõe para stories e reels

## O que esperar da resposta

1. Um comentário curto com a ideia central.
2. O prompt completo, em inglês, num único bloco de código. É só copiar e colar no gerador de vídeo.
3. Por que a ideia funciona, qual o elemento mais arriscado para o gerador e o Plano B mais simples.

## Dicas

- **Textos na tela são o ponto fraco dos geradores de vídeo.** Se as letras saírem deformadas, gere o vídeo sem texto (peça isso à skill) e coloque os textos depois, num editor.
- **Os vídeos saem sem som por padrão.** É uma regra da skill: o ritmo vem do movimento. Coloque trilha na edição, se quiser.
- **Confira números e fatos.** Se o vídeo tiver dados reais (preços, datas, estatísticas), verifique antes de publicar.
